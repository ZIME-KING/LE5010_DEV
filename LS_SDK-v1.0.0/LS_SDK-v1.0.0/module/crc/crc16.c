...
static const uint16_t cavan_crc16_table[] = {
...
uint16_t cavan_crc16_update_byte(uint16_t crc, uint8_t value)
{
	return cavan_crc16_table[(crc ^ value) & 0xFF] ^ (crc >> 8);
}

uint16_t cavan_crc16_update(uint16_t crc, const uint8_t *buff, uint16_t length)
{
	const uint8_t *buff_end;

	for (buff_end = buff + length; buff < buff_end; buff++) {
		crc = cavan_crc16_update_byte(crc, *buff);
	}
...
{
	return cavan_crc16_update(0, buff, length);
}
