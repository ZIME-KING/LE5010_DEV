...
void Start_Lock_Send() {
    ...
    DATA_BUF[20] = !lock_state[1];	//lock_state
		...
		DATA_BUF[23] = 0xA8;			//MAC_ADDR
		memcpy(&DATA_BUF[24],&MAC_ADDR[0],6);

		DATA_BUF[30] = 0xA7;			//MICI 卡号
		memcpy(&DATA_BUF[31],&CIMI_DATA[0],15);
		
		DATA_BUF[46] = 0xA9;			//MICI 设备号
		memcpy(&DATA_BUF[47],&MICI_DATA[0],15);
		
		DATA_BUF[62] = 0x81;			//RFID
		memcpy(&DATA_BUF[63],&RFID_DATA[0],4);

		DATA_BUF[67] = 0x82;			//RFID
		memcpy(&DATA_BUF[68],&RFID_DATA_2[0],4);

			
    temp=CRC16_8005Modbus(&DATA_BUF[0],72);
    DATA_BUF[73]=(temp & 0xff00) >>8;
    DATA_BUF[72]= temp & 0xff;
...
void Open_Lock_Send() {
    ...
    DATA_BUF[18] = !lock_state[1];	//lock_state
		...
		memcpy(&DATA_BUF[22],&MICI_DATA[0],15);

    temp=CRC16_8005Modbus(&DATA_BUF[0],37);
    DATA_BUF[38]=(temp & 0xff00) >>8;
    DATA_BUF[37]= temp & 0xff;
...
void Open_Lock_Data_Send() {
   uint8_t RX_BUF[100];
    ...
    DATA_BUF[18] = !lock_state[1];	//lock_state
		...
		DATA_BUF[37] = 0x81;			//RFID
		memcpy(&DATA_BUF[38],&RFID_DATA[0],4);
		
		DATA_BUF[42] = 0x82;			//RFID
		memcpy(&DATA_BUF[43],&RFID_DATA_2[0],4);

    temp=CRC16_8005Modbus(&DATA_BUF[0],47);
    DATA_BUF[48]=(temp & 0xff00) >>8;
    DATA_BUF[47]= temp & 0xff;
