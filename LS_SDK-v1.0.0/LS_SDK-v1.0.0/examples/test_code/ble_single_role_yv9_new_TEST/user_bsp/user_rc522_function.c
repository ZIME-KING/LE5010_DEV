...
//		rfid_task_flag_2=0;
    ...
    if(rfid_task_flag_2==0 ) {
        if(once_flag!=0xAA) {
           // RC522_GPIO_INIT_2();
            ...
            status=M500PcdConfigISOType( 'A' );
						...
						if(user_test==0x80){    //初始化成功，设备存在，开启扫卡任务
									LOG_I("FIND_RFID_DRIVE2");
									rfid_task_flag_2=0;
						}
						else{             		//初始化失败，设备不存在，不开启扫卡任务
									LOG_I("NOT_FIND_RFID_DRIVE2");
									rfid_task_flag_2=1;
						}
            once_flag=0xAA;
						return 1;
        ...
        if(count>20) {						
						...
						if((RFID_DATA_2[0]+RFID_DATA_2[1]+RFID_DATA_2[2]+RFID_DATA_2[3])!=0){
						RFID_DATA_2[0]=0;RFID_DATA_2[1]=0;RFID_DATA_2[2]=0;RFID_DATA_2[3]=0;						
						LOG_I("NOT_FIND_CARD2");
...
//
																		...
																		RFID_DATA_2[0]=SelectedSnr[0];
																		RFID_DATA_2[1]=SelectedSnr[1];
																		RFID_DATA_2[2]=SelectedSnr[2];
																		RFID_DATA_2[3]=SelectedSnr[3];		
																		memcpy(RFID_DATA_2,SelectedSnr,4);																		
																		LOG_HEX(&RFID_DATA_2[0],4);
																		LOG_I("FIND_CARD2");
