...
//	unsigned char pt[16]=  { 0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f,0x10 };
//	unsigned char pt[16] = {0xc6,0x53,0x0c,0x1d,0x64,0x30,0xff,0xB8,0x6D,0x17,0x28,0xa5,0x86,0xdd,0x3c,0x1c};
//	unsigned char  key[16]={ 0x58,0x49,0x4e,0x4a,0x59,0x55,0x45,0x5a,0x4e,0x2e,0x43,0x4f,0x4d,0x30,0x35,0x31 };
...
//	u3_printf("after Contrary_AesEncrypt,plaintext is: ");//将解密文件输出
//	for (i = 0; i < 16; i++)
//	{
//		u3_printf("0x%x  ", pt[i]);
//	}
...
//	int j;
    ...
    AesEncrypt(pt,expansionkey,10);		//2、AES 加密
#ifdef AES_DUBUG
    HAL_UART_Transmit_IT(&UART_Config,(unsigned char*)"生成的密文为: ",sizeof("生成的密文为: "));
    for (i = 0; i < 16; i++)
    {
        sprintf((char*)&TEMP_BUF[i*3],"%02x ", pt[i]);
    }
    _t();
    HAL_UART_Transmit_IT(&UART_Config,(unsigned char*)&TEMP_BUF[0],strlen((char*)&TEMP_BUF[0]));
#endif
}
...
#ifdef AES_DUBUG
    HAL_UART_Transmit_IT(&UART_Config,(unsigned char*)"解密的密文为: ",sizeof("解密的密文为: "));
    for (i = 0; i < 16; i++)
    {
        sprintf((char*)&TEMP_BUF[i*3],"%02x ", pt[i]);
    }
...
void User_Encryption(unsigned char *in_data,unsigned char *out_data, unsigned char len) {
    memcpy(&pt[0],in_data,len);
    aes_jia_mi();
    memcpy(out_data,&pt[0],len);
}
//解密
//输入数据地址
//输出数据地址
