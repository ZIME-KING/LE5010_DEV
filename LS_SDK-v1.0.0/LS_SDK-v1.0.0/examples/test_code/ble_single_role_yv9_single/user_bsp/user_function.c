...
volatile static uint16_t once_flag=0;
...
void State_Change_Task(){
	static uint8_t count;
	volatile static uint8_t sw_count=0;
	volatile static uint8_t sw_flag=0;			
	volatile static uint8_t sw_count_2=0;
	volatile static uint8_t sw_flag_2=0;			
			count++;
			...
			if(last_lock_state != lock_state[0] && moro_task_flag!=1 ){
