...
uint8_t C0_lock_state[LOCK_NUM];  //锁状态存储 防盗锁，
...
void WAKE_UP(){
    ...
    io_cfg_output(PB08);  //
    io_write_pin(PB08,1);	//
		DELAY_US(500*1000);   //开启cat1 mos管等待500ms电源稳定
...
//    DATA_BUF[20] = !lock_state[1];	//lock_state		
		  ...
		  DATA_BUF[17] = 0x00; 	  //lock_ID
			DATA_BUF[18] = !lock_state[0];	//lock_state
		
			DATA_BUF[19] = 0xC0; 	  //lock_ID
...
//			memcpy(&DATA_BUF[66],&RFID_DATA_2[0],4);
    ...
    temp=CRC16_8005Modbus(&DATA_BUF[0],60);
    DATA_BUF[61]=(temp & 0xff00) >>8;
    DATA_BUF[60]= temp & 0xff;
