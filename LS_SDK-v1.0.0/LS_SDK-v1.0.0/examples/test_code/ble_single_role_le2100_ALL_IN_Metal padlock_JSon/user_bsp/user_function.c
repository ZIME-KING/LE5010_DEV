...
uint16_t length_20 = 20;
uint16_t length_15_1 = 15;
...
//uint8_t temp_val = 0xAA;
	...
	if(strncmp("ok",(char*)tmp,sizeof("ok"))!=0){
		...
		tinyfs_write(ID_dir_3,RECORD_KEY9,(uint8_t*)&CIMI_DATA[0],15);	
		tinyfs_write(ID_dir_3,RECORD_KEYA,(uint8_t*)&CCID_DATA[0],20);	
...
//		tinyfs_write(ID_dir_3,RECORD_KEY_T,(uint8_t*)&temp_val,1);	//给测试模式标记成0xBB（不开启）
	...
	LOG_I("KEY9_CIMI");
	LOG_I("CIMI,%15s",&CIMI_DATA[0]);
	
	tinyfs_read(ID_dir_3,RECORD_KEYA,tmp,&length_20);//读到tmp中
	memcpy (&CCID_DATA[0], &tmp[0],20);
	LOG_I("CCID_DATA");
	LOG_I("CCID_DATA,%20s",&CCID_DATA[0]);
	
	
	tinyfs_read(ID_dir_3,RECORD_KEYB,tmp,&length_15_1);//读到tmp中
	memcpy (&IMEI_DATA[0], &tmp[0],15);
	LOG_I("IMEI_DATA");
	LOG_I("IMEI_DATA,%15s",&IMEI_DATA[0]);
...
const char OPEN_LOCK_KEY[]={0x00,0x02,0x00,0x11,0x00,0x02,0x0f,0x7b,0x22,0x64,0x02,0x65,0x63,0x02,0x6f,0x64,0x65,0x02,0x69,0x02,0x64,0x22,0x3A,0x20,0x34,0x7D};
    ...
    if(frame_2[uart_2_frame_id].status!=0){						//接收到数据后status=1;
        //HAL_UART_Transmit(&UART_Config,(uint8_t*)frame_2[uart_2_frame_id].buffer,frame_2[uart_2_frame_id].length,100);
          ...
          if(frame_2[uart_2_frame_id].buffer[count]=='E'){
				...
				//收到ccid				
				else if( strncmp("AT+ECICCID\r\n+ECICCID:",(char*)frame_2[uart_2_frame_id].buffer,strlen("AT+ECICCID\r\n+ECICCID:"))==0){
            //HAL_UART_Transmit(&UART_Config,(uint8_t*)"error \r\n",sizeof("error \r\n"),10);
        
					count=strlen("AT+ECICCID\r\n+ECICCID:");
          if(frame_2[uart_2_frame_id].buffer[count]=='E'){
						globle_Result=CIMI_ERROR;  //错误标记  这里标记不是这个要测试
					}
					else{
						globle_Result=CCID_OK;
                ...
                if(frame_2[uart_2_frame_id].buffer[i]==',') {
                    ...
                    break;
									...
									//memcpy (&frame_2[uart_2_frame_id].buffer[count], &AT_RX_DATA_BUF[0],sizeof(OPEN_LOCK_KEY));
									 //是OPEN_LOCK
									 
									// LOG_I();
//									if(strncmp((char*)AT_RX_DATA_BUF,(char*)OPEN_LOCK_KEY,sizeof(OPEN_LOCK_KEY))==0){								   
//										KEY_FLAG=1;
            ...
            for(int i=0; i<frame_2[uart_2_frame_id].length-count; i++) {
                ...
                if(frame_2[uart_2_frame_id].buffer[count+1+i*2]<='9')
                    ...
                    AT_RX_DATA_BUF[i]+=(frame_2[uart_2_frame_id].buffer[count+1+i*2]-'A'+10);
							 ...
							 LOG_HEX(AT_RX_DATA_BUF,sizeof(OPEN_LOCK_KEY));
							if(strncmp((char*)AT_RX_DATA_BUF,(char*)OPEN_LOCK_KEY,sizeof(OPEN_LOCK_KEY))==0){								   
										KEY_FLAG=1;
										open_lock_reply_Result=1;
							}
...
//            }
            ...
            globle_Result=OK_AT;
				...
				else if( strncmp("AT+CTM2MSETPM=117.60.157.137,5683,86400\r\nOK",(char*)frame_2[uart_2_frame_id].buffer,strlen("AT+CTM2MSETPM=117.60.157.137,5683,86400\r\nOK"))==0){
            //HAL_UART_Transmit(&UART_Config,(uint8_t*)"error \r\n",sizeof("error \r\n"),10);
            globle_Result=OK_AT;
        }
...
//                send_count++;
                ...
                if(count==1) {
									count=0;
										LOG_I("5_REPLY_SEND_REPLY_OK_TIME_OUT");
                    Set_Task_State(REPLY_SEND,STOP);
                    temp=TIME_OUT;
                }
            }
...
//					tinyfs_write(ID_dir_2,RECORD_KEY1,SHORT_NAME,sizeof(SHORT_NAME));	
					...
					memcpy(&IMEI_DATA[0],&AT_RX_DATA_BUF[0],15);
					tinyfs_write(ID_dir_3,RECORD_KEYB,IMEI_DATA,sizeof(IMEI_DATA));	
					
					
					tinyfs_write_through();
					//LOG_I("W_SHORT_NAME_OK %s",SHORT_NAME);		
					LOG_I("W_IMEI_DATA_OK %s",IMEI_DATA) ;
			...
			case 8:
			if(Get_Uart_Data_Processing_Result()==CCID_OK){
				globle_Result=0xff;
				
				memcpy(&CCID_DATA[0],&AT_RX_DATA_BUF[0],20);
				LOG_I("WIRT_OK %20s",CCID_DATA);	
				
				
				tinyfs_write(ID_dir_3,RECORD_KEYA,CCID_DATA,20);	
				tinyfs_write_through();	
